 ...
 * Step 3: "42" ("42" is read in) ^ The parsed integer is 42. Since 42 is in the
 * range [-231, 231 - 1], the final result is 42.
 * 
 * Example 2: Input: s = " -42" Output: -42 Explanation: Step 1: " -42" (leading
 * whitespace is read and ignored) ^ Step 2: " -42" ('-' is read, so the result
 * should be negative) ^ Step 3: " -42" ("42" is read in) ^ The parsed integer
 * is -42. Since -42 is in the range [-231, 231 - 1], the final result is -42.
 * 
 * Example 3: Input: s = "4193 with words" Output: 4193 Explanation: Step 1:
 ...
 * 
 * Constraints: - 0 <= s.length <= 200 - s consists of English letters
	...
	public int myAtoi(String s) {
		...
		if (s != null && !s.isEmpty()) {
			...
			if (i < s.length() && (s.charAt(i) == '-' || s.charAt(i) == '+')) {
				sign = 1 - 2 * (s.charAt(i) == '-' ? 1 : 0); // -1 or 1
				i++;
			}
			...
			while (i < s.length() && s.charAt(i) >= '0' && s.charAt(i) <= '9') {
				...
				if (atoi > Integer.MAX_VALUE / 10 || (atoi == Integer.MAX_VALUE / 10 && s.charAt(i) - '0' > 7)) {
					if (sign == 1) {
