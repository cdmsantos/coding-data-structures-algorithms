 ...
 * s = '{[(])}' {{[[(())]]}} third s ='{{[[(())]]}}' Sample Output
 * 
 * YES NO YES Explanation
 * 
 * The string {[()]} meets both criteria for being a balanced string. The string
 * {[(])} is not balanced because the brackets enclosed by the matched pair {
 * and } are not balanced: [(]). The string {{[[(())]]}} meets both criteria for
 * being a balanced string.
 * 
 */
public class BalancedBracketsHackerRank {
	public static String isBalanced(String s) {
		...
		while (currentIndex < s.length()) {
			...
			if (current == '[' || current == '{' || current == '(') {
				...
				if ((current == ']' && lastBracket == '[') || (current == '}' && lastBracket == '{')
						|| (current == ')' && lastBracket == '(')) {
					brackets.pop();
				} else {
					return balanced;
	...
	public static void main(String[] args) {
		System.out.println(BalancedBracketsHackerRank.isBalanced("}][}}(}][))]")); // NO
		System.out.println(BalancedBracketsHackerRank.isBalanced("[](){()}")); // YES
		System.out.println(BalancedBracketsHackerRank.isBalanced("()")); // YES
		System.out.println(BalancedBracketsHackerRank.isBalanced("({}([][]))[]()")); // YES
		System.out.println(BalancedBracketsHackerRank.isBalanced("{)[](}]}]}))}(())(")); // NO
		System.out.println(BalancedBracketsHackerRank.isBalanced("([[)")); // NO
